...
const SectionsIntake = () => {
  ...
  const handleAddSection = () => {
    ...
    console.log(
      "Added a new section. Total sections: " + sectionComponents.length
    );
  };

  const handleRemoveSection = (sectionIndexToRemove) => {
    setSectionComponents(
      sectionComponents.filter((_, index) => index !== sectionIndexToRemove)
    );
    console.log(
      "Remove Section " +
        sectionIndexToRemove +
        " called. Remaining sections - " +
  ...
  const handleSubmit = () => {
    ...
    navigate("/ReviewAddRestaurant", {
      state: { restaurant: restaurant, sections: sectionComponents },
    });
  };
  ...
  const setSecImgs = (index, secImg) => {
    const prop = "secImg";
    const sectionComponentsList = [...sectionComponents];
    sectionComponentsList[index][prop] = secImg;
    console.log(prop + " : " + sectionComponentsList[index][prop].length)
    setSectionComponents(sectionComponentsList);
  ...
  return (
    <Container>
      ...
      {sectionComponents.map((sectionComponent, index) => (
        <AddSection
          key={index}
          sectionName={sectionComponent.sectionName}
          sectionDescription={sectionComponent.sectionDescription}
          capacity={sectionComponent.capacity}
          avgCost={sectionComponent.avgCost}
          secImg={sectionComponent.secImg}
          dineinAvailable={sectionComponent.dineinAvailable}
          reservationCharge={sectionComponent.reservationCharge}
          cateringAvailable={sectionComponent.cateringAvailable}
          cuisines={sectionComponent.cuisines}
          searchtags={sectionComponent.searchtags}
          timing={sectionComponent.timing}
          autoAcceptBookings={sectionComponent.autoAcceptBookings}
          menu={sectionComponent.menu}
          onRemove={() => handleRemoveSection(index)}
          onChange={(event) => handleSectionChanges(event, index)}
          setTimings={(timing) => setTimings(timing, index)}
          setSecImg={(secImg) => setSecImgs(index, secImg)}
          setMenu={(menu) => setMenu(menu, index)}
        />
      ))}
      <Button
        variant="contained"
